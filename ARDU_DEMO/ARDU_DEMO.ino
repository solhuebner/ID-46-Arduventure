...
bool checkSpaceFree(int8_t offsetX, int8_t offsetY)
{
  ...
  if(player.currentMap > 27)
  {
    if((player.x + 2 + offsetX) < 0 || (player.x + 14 + offsetX) > 128) return false;
    else if((player.y + 7 + offsetY) < 0 || (player.y + 16 + offsetY) > 64) return false;
  }
...
void updateScene()
{
  camX = clamp(player.x - 56, 0, 2048 - 128);
  camY = clamp(player.y - 24, 0, 2048 - 64);
